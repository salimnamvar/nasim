...
INFRA_LABEL = {
    "session": "FileSystem", "tool": "FileSystem", "agent": "FileSystem",
    "api": "FileSystem", "cli": "FileSystem", "editstrategy": "FileSystem",
    "evaluation": "TestSuite", "config": "FileSystem", "contextgraph": "TokenBudget",
    "git": "GitRepository", "hooks": "HookRuntime", "mcp": "MCPServer",
    "memory": "FileSystem", "observability": "MetricsBackend", "plugins": "PluginRuntime",
    "provider": "LLMBackend", "repointelligence": "FileSystem", "repointer": "FileSystem",
    "router": "ModelBackend", "safety": "PolicyEngine", "sandbox": "HostShell",
    "wirelog": "FileSystem",
}
...
def fix_file(path: Path) -> bool:
    ...
    while i < n:
        ...
        if re.match(r'participant\s+"[^"]*API"\s+as\s+api\s*$', s) or \
           re.match(r'participant\s+"[^"]*Controller"\s+as\s+ctrl\s*$', s):
            i += 1
            continue
