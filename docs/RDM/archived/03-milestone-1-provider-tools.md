...
- Provider Protocol + LLM Repository + OllamaProvider
- Tool ABC + Tool Service + all tool implementations
- MCP Repository for extension tools

## Deliverables

| # | Deliverable | UC Trace | SQ Trace |
| - | --- | --- | --- |
| 1 | `Provider` Protocol + `LLM Repository` + `LLMResponse` + `ToolCall` | LLMREPOSITORY-01, LLMREPOSITORY-04 | sq_prv01, sq_prv04 |
| 2 | `OllamaProvider` (chat + stream) | LLM-01, LLM-02 | sq_llm01, sq_llm02 |
| 3 | `Tool` ABC + `Tool Service` + `ToolResult` | TOOLSERVICE-01..12 | sq_tl01..12 |
...
| 8 | `MCP Repository` | TOOLSERVICE-12 | sq_tl12 |
...
- Tool Service supports dynamic registration (for MCP)
- MCP Repository wraps MCP server tools into nasim Tool format
