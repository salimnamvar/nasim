...
- AgentOrchestrator (core loop: LLM call → tool dispatch → repeat)
- ConversationHistory (messages + token tracking)
- ContextCompactor (summarize old exchanges when budget exceeded)
- PermissionGate (per-tool safety: ask/auto/off)
- PlanSession (queue tool calls in plan mode)
...
| - | --- | --- | --- |
| 1 | `AgentOrchestrator` core loop | AGT-01 | sq_agt01 |
...
| 4 | `ContextCompactor` | AGT-06, CTX-02, CTX-03 | sq_agt06, sq_ctx02, sq_ctx03 |
| 5 | `PermissionGate` | SAF-01, SAF-02, SAF-03 | sq_saf01, sq_saf02, sq_saf03 |
| 6 | `PlanSession` | AGT-07, AGT-08 | sq_agt07, sq_agt08 |
...
- PermissionGate blocks unsafe tools in `ask` mode, prompts user
- ContextCompactor triggers when token_count > budget, summarizes oldest exchanges
- PlanSession queues tool calls, drains on /approve
