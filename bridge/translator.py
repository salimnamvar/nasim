...
Functions:
    ...
    extract_required_defaults: Build per-tool required-field default map.
...
def extract_required_defaults(a_tools: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """Build a per-tool map of required-field → zero-value default.

    Smaller models (e.g. gemma4, qwen2.5-coder) routinely omit non-semantic
    required fields such as Claude Code's ``description`` field on the Bash
    tool. The client validates the returned tool call against its own schema
    and rejects it with "Invalid tool parameters" when required fields are
    absent. Filling in zero-value defaults before returning the response to the
    client makes the call pass validation without distorting the actual output.

    Args:
        a_tools (List[Dict[str, Any]]): Anthropic tool definitions from the
            incoming request.

    Returns:
        Dict[str, Dict[str, Any]]: ``{tool_name: {field_name: default_value}}``
        for every required field in every tool.
    """
    ...
    for tool in a_tools:
        ...
        for field in required:
            ptype = props.get(field, {}).get("type", "string")
            defaults[field] = _type_defaults.get(ptype, "")
...
def salvage_tool_calls(
    a_text: str, a_tool_names: List[str]
...
def _tool_use_block(
    a_call: Dict[str, Any],
    a_required_defaults: Optional[Dict[str, Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    """Convert one Ollama tool_call entry to an Anthropic tool_use block.

    Args:
        a_call (Dict[str, Any]): Ollama tool_call dict with a ``function`` key.
        a_required_defaults (Optional[Dict[str, Dict[str, Any]]]): Per-tool
            required-field defaults from :func:`extract_required_defaults`.
            When provided, any required field absent from the model output is
            filled with its zero-value default so client schema validation passes.

    Returns:
        Dict[str, Any]: Anthropic ``tool_use`` content block.
    """
    ...
    if a_required_defaults and name in a_required_defaults:
        filled: Dict[str, Any] = dict(a_required_defaults[name])
...
def ollama_to_anthropic(
    ...
    Args:
        ...
        a_required_defaults (Optional[Dict[str, Dict[str, Any]]]): Per-tool
            required-field defaults from :func:`extract_required_defaults`.
            Fills fields that the model omitted so client validation passes.
    ...
    for call in tool_calls:
        content.append(_tool_use_block(call, a_required_defaults))
...
async def stream_ollama_to_anthropic(
    ...
    for call in tool_calls:
        saw_tool_calls = True
        index += 1
        for event in _block_events(index, _tool_use_block(call, a_required_defaults)):
            yield event
