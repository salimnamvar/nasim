...
async def messages(a_request: Request) -> Any:
    ...
    tool_names = [t["name"] for t in body.get("tools", []) if t.get("name")]
    ...
    if not ollama_body["stream"]:
        ...
        return JSONResponse(translator.ollama_to_anthropic(resp.json(), requested_model, tool_names))
            ...
            async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
                async with client.stream("POST", f"{OLLAMA_URL}/api/chat", json=ollama_body) as resp:
                    ...
                    async for event in translator.stream_ollama_to_anthropic(
                        _ollama_chunks(resp), requested_model, tool_names
                    ):
                        yield event
        except httpx.HTTPError as exc:
            yield translator._sse(
